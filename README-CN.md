...
- **知识图谱**：普通项目保留事件-实体图；类型化血缘使用白色画布的 Three.js 3D 图，任务、表、字段节点和标签可分别显隐，任务-任务、任务-表、表-表、表-字段、字段-字段关系可独立筛选。
...
在「图谱」页可以查看项目关系。类型化血缘项目使用可旋转、缩放和拖动的 3D 任务-表-字段图，支持搜索、点击聚焦并增量展开、详情、适配视图，以及节点、名称标签和五类关系的独立显隐控制。

![SAG 图谱视图](docs/assets/sag-graph.png)

服务运行时可执行 `npm run verify:lineage-ui` 重复视觉验收；脚本会检查文档类型筛选、任务依赖、WebGL 像素、图谱开关和桌面/移动端溢出。
