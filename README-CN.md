...
- **文档处理结果**：查看切片、事项、实体、Embedding 数据，支持标题关键字、事件类型、实体类型筛选和分页浏览。
...
- **知识图谱**：普通项目保留事件-实体图；类型化血缘使用白色画布的 Three.js 3D 图，任务、表、字段节点和标签可分别显隐，五类关系可独立筛选；点击实体后可按 `1-5` 层只高亮直接或间接关联链路。
- **MCP 接入**：每个项目都有自己的 MCP 配置，外部 Agent 可直接调用当前项目资料。

## 技术栈

SAG 使用 TypeScript 贯穿前后端。前端是 React + Vite + Tailwind CSS 的 WebUI，类型化血缘图按需加载 Three.js/WebGL；后端是 Fastify HTTP API、MCP TypeScript SDK 和分层服务模块；数据层使用 PostgreSQL、pgvector、全文检索和 SQL 多跳查询；模型侧兼容 OpenAI-compatible 的 LLM、Embedding 和 Rerank 接口。
...
在「图谱」页可以查看项目关系。类型化血缘项目使用可旋转、缩放和拖动的 3D 任务-表-字段图，支持搜索、点击聚焦、`1-5` 层上下游穿透、增量展开、详情、适配视图，以及节点、名称标签和五类关系的独立显隐控制。选中实体时只显示当前层级内的关联关系，无关节点降灰；点击空白或关闭详情后恢复完整图。

![SAG 图谱视图](docs/assets/sag-graph.png)

服务运行时可执行 `npm run verify:lineage-ui` 重复视觉验收；脚本会检查文档类型筛选、任务依赖、1/2 层穿透高亮、空白恢复、WebGL 像素、图谱开关和桌面/移动端溢出。
